...
│  └──────────────────┘                                │
│  ▲
│  │ clean move-out (full refund) → tenant reputation
│  └─────────── rental_agreement writes back
└──────────────────────────────────────────────────────┘
...
| `user_registry`    | Identity directory: wallet → role (Landlord/Tenant/Arbitrator) + reputation. Consulted by dispute to verify arbitrators, and receives reputation updates from two sources: the dispute contract (winner/loser after settlements) and the rental agreement contract (tenant after a clean move-out). Never touches funds. |
...
accept_deduction(env, agreement_id: u32, tenant: Address) -> Result<(), Error>  // → escrowopen_dispute(env, agreement_id: u32, initiator: Address, reason: String)
             -> Result<(), Error>                                               // → dispute

close_agreement(env, agreement_id: u32, caller: Address) -> Result<(), Error>
             // → escrow (release) and, on a full refund, → registry
             //   (clean move-out boosts tenant reputation)
```
...
set_reputation_source(env, admin: Address, source: Address) -> Result<(), Error>  // additive
remove_reputation_source(env, admin: Address, source: Address) -> Result<(), Error>
adjust_reputation(env, caller: Address, user: Address, delta: i32) -> Result<(), Error>
                                                                    // caller ∈ reputation sources
...
| `UserRegistry`                      | `Address` (wiring — clean move-out reputation)                                              |
...
| ------------------- | ---------------------------------------------------------------------------------------- |
| `Admin`             | `Address`                                                                                |
| `ReputationSources` | `Vec<Address>` — the contracts allowed to call `adjust_reputation` (dispute + agreement) |
