        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-5 max-w-xl text-base leading-relaxed text-ink-500 sm:text-lg">
              TrustRent holds rental security deposits in a smart-contract escrow.
              Tenants know their money is safe; landlords get an evidence-backed
              move-out process. No more he-said, she-said over a deposit.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Link href="/dashboard">
                <Button size="lg">View demo dashboard</Button>
              </Link>
              <a href="#how-it-works" className="inline-flex h-12 items-center px-2 text-sm font-medium text-ink-700 hover:text-ink-900">
                How it works
              </a>
            </div>
      ...
      <section className="border-y border-border bg-ivory-50/70">
        <div className="mx-auto max-w-6xl px-6 py-12">
          ...
          <ol className="mt-6 flex flex-wrap items-center justify-center gap-x-2 gap-y-3 text-xs text-ink-500">
            {["Created", "Active", "Move-out requested", "Evidence submitted", "Inspection pending", "Approved", "Disputed", "Settlement", "Closed"].map(
              (step, i) => (
      ...
      <section className="border-t border-border bg-surface">
        <div className="mx-auto max-w-6xl px-6 py-16 sm:py-20">
          <div className="grid gap-8 sm:grid-cols-3">
            <WhyBlock icon={<IconShield className="size-5" />} title="Fair by default" text="A neutral smart contract, not one party's word, controls the money." />
            <WhyBlock icon={<IconCheck className="size-5" />} title="Transparent by design" text="Every event — lock, move-out, evidence, release — is on-chain and visible." />
            <WhyBlock icon={<IconScale className="size-5" />} title="Dispute-ready" text="If sides disagree, the deposit stays locked while evidence is reviewed." />
          </div>
        </div>
      </section>
...
function HeroLine({ ok = false, pending = false, label }: { ok?: boolean; pending?: boolean; label: string }) {
  return (
